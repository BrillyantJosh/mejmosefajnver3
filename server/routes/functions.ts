  ...
  try {
    ...
    db.prepare(`
      INSERT INTO kind_38888 (
        ...
        exchange_rates, split, version, valid_from, split_started_at, split_target_lana, trusted_signers, raw_event
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      'synced_' + Date.now(),
      data.event_id,
...
router.post('/publish-dm-event', async (req: Request, res: Response) => {
  try {
    ...
    // ⚠️ Do NOT reduce below 60000ms — causes "Sending failed" for audio/larger payloads
    const results = await publishEventToRelays(relays, event, 60000);
...
// The Lana.Discount external-sale proxy used to live here. It forwarded a sale
// straight to /api/external/sale with a rate this app chose. Lana.discount now
// decides whether to acquire before any LANA moves, so there is nothing here to
// forward: selling starts at https://lana.discount/offer.
