...
function ProposalCard({
  ...
  return <Card className="cursor-pointer hover:shadow-lg transition-shadow border-border/50 hover:border-primary/30" onClick={onClick}>
      {proposal.img && <div className="w-full h-40 overflow-hidden rounded-t-lg">
      ...
      <CardHeader className="pb-2">
        <div className="flex items-start justify-between gap-2">
          ...
          <Badge variant={proposal.level === 'global' ? 'default' : 'secondary'} className="shrink-0">
            {proposal.level === 'global' ? <><Globe className="h-3 w-3 mr-1" /> Global</> : <><MapPin className="h-3 w-3 mr-1" /> Local</>}
          </Badge>
        </div>
      </CardHeader>
      <CardContent className="space-y-3">
        ...
        <div className="flex items-center gap-4 text-xs text-muted-foreground">
          ...
          {hasMedia && <div className="flex items-center gap-2">
...
export default function ActiveAlignments() {
  ...
  return <div className="p-4">
      ...
      {isLoading ? <LoadingSkeleton /> : error ? <div className="text-center py-12 text-destructive">
          <p>{error}</p>
        </div> : proposals.length === 0 ? <div className="text-muted-foreground text-center py-12">
          ...
          {proposals.map(proposal => <ProposalCard key={proposal.id} proposal={proposal} onClick={() => setSelectedProposal(proposal)} />)}
        </div>}
    </div>;
}